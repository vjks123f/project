...
if (isset($_POST['submit'])) {
    $new_name = mysqli_real_escape_string($con, $_POST['new-name']);
    $new_email = mysqli_real_escape_string($con, $_POST['new-email']);

    mysqli_query($con, "UPDATE user SET name='$new_name' 
    , email ='$new_email'") or die('query failed');

    $old_pass = $_POST['old-pass'];
    $user_old_pass = mysqli_real_escape_string($con, md5($_POST['user-old-pass']));
    $new_pass = mysqli_real_escape_string($con, md5($_POST['new-pass']));
    $confirm_new_pass = mysqli_real_escape_string($con, md5($_POST['confirm-new-pass']));
    ...
    if(!empty($user_old_pass) || !empty($new_pass) || !empty($confirm_new_pass)){
        if($old_pass != $user_old_pass){
            ...
            mysqli_query($con, "UPDATE user SET password='$new_pass' 
    ...
    if(!empty($image)){
        if ($image_size > 2000000) {
            ...
            $image_query= mysqli_query($con, "UPDATE user SET image='$image' 
                            WHERE id='$user_id'") or die('query failed');
        ...
        <div class="update-profile">
            <?php
                ...
                if($fetch['image'] == ''){
                    ...
                    echo "<img src='upload_images/".$fetch['image']."'>";
                }
